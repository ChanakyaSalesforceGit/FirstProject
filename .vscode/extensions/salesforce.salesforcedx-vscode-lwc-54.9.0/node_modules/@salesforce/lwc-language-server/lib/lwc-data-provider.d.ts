import { IAttributeData, ITagData, IValueData, IHTMLDataProvider } from 'vscode-html-languageservice';
import ComponentIndexer from './component-indexer';
export declare type DataProviderAttributes = {
    indexer: ComponentIndexer;
};
export declare class LWCDataProvider implements IHTMLDataProvider {
    activated: boolean;
    indexer?: ComponentIndexer;
    private readonly _standardTags;
    private readonly _globalAttributes;
    constructor(attributes?: DataProviderAttributes);
    getId(): string;
    isApplicable(): boolean;
    provideTags(): ITagData[];
    provideAttributes(tagName: string): IAttributeData[];
    provideValues(): IValueData[];
}
//# sourceMappingURL=lwc-data-provider.d.ts.map